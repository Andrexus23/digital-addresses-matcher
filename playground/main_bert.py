...
with Session(bind=engine) as session:
    addresses = AddressService(session).get_addresses_where(limit=500)
    addresses_strs = [addr.epgu_address for addr in addresses]
    # addresses_strs = ['183036, обл. Мурманская, г. Мурманск, ул. Капитана Маклакова, д. 6, к. 1, стр. 1, кв. 57']
    matcher = AddressMatcher(session)
    start_time = time.time()
    parsed_addresses_with_tails = matcher.match(addresses_strs, model_case=ModelEnum.BERT_NER_CASE)
    end_time = time.time()
